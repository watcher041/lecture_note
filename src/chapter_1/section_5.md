...
これは座標を回転させたときに得られる変換式と類似したものとなっている。実際、原点を軸に座標系 $(x,y)$ を角度 $\alpha$ だけ回転させて座標系 $(x',y')$ に変換した場合、
...
　以上のように、回転運動を取り扱うには通常の運動方程式とは異なる方程式を利用する必要があることが分かる。ただ、このとき直交座標系では出ず極座標になったときに現れた遠心力について見ていないため、一度こちらに関して述べていくことにする。
