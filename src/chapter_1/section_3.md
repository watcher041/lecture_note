...
$$
    ...
    \frac{Gm_1m_2\mu}{L^2}
$$
そのため、$r$ は以下のように表される（$\epsilon$：離心率、$d$：焦点距離）。
$$
    r=
    \frac{d\epsilon}
...
$$
この軌道は離心率 $\epsilon$ の大きさによって決まる。この大きさの比はどういうことことかいうと $L$ を展開することで以下のような形になることが分かる。
$$
    \epsilon=
    \frac{\mu r\dot{\theta}^2}
    {G\frac{m_1m_2}{r^2}}\frac{r}{d}
$$
これを見ると、一つの目の比は遠心力と万有引力の比となっており、もう一つは距離が焦点距離からどれだけ離れているか表している。そのため、これらのパターンに分けて考える必要がある。
