...
では、物体を連続体とみなすと運動はどのように記述されるかを見てみることにしよう。この場合も１つの質点に関する運動方程式は $i$ 番目について
...
となっている。ここで、$m_i$ は質点の質量、 $\bm{r}_i$ は質点の位置、$\bm{F}_{i}$ は質点に働く外力、$\bm{F}_{ij}$ は $j$ 番目の質点との間に働く力としている。ただ、先ほどの述べた通り質点が連続して分布していることを踏まえるとこれらの物質量は位置 $\bm{r}$ に対して連続した値で記述できるものと思われる。そこで、試しに $m_i$ を $\rho(\bm{r})$ というように $\bm{r}$ の関数として表示してみると、全体の質量 $m$ は
...
というように物体内の全領域 $V$ の積分で表示される。同じように、$\bm{F}_{i}$ に対しても $\bm{f}(\bm{r})$ と置くことで全体の外力 $\bm{F}$ も

$$
  \bm{F}=\int_V \bm{f}(\bm{r})\ 
  \mathrm{d}\bm{r}
...
$$
  m_i\leftrightarrow \rho(\bm{r})、
  \bm{F}_{i}\leftrightarrow \bm{f}(\bm{r})、
  \bm{F}_{ij}\leftrightarrow\bm{p}(\bm{r},\bm{r}')
$$

このうち、 $\bm{p}(\bm{r},\bm{r}')$ としているのは $\bm{r}'$ に対しての総和が

$$
  \sum_{j=1(j\neq i)}^{N}\bm{F}_{ij}
  \leftrightarrow
  \int_{V}\bm{p}(\bm{r},\bm{r}')\ 
  \mathrm{d}\bm{r}'
$$

というように物体内の範囲 $V$ に対する積分で記述されるとき次元として面積で割る形になっているためである。
