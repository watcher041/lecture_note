...
$$
というようになるが、ここで \\(v_x'\\) に着目すると \\(v_x\\) に対して以下の条件下でないと発散することが分かる。
$$
    v_x<\frac{c^2}{V}
$$
...
$$
今回もこれと同様の式が成立しているものとして、質量がどのように速度に依存しているか確認してみることにする。
...
\と表される。一方で、同じ状況をK$'$のように動いて観測した場合には と書ける。この時、$\bm{r}'$は観測者Kが扱う座標により \begin{equation}
