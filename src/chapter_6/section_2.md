...
　気体の性質を調べる際には、よくピストンなどの容器に気体を閉じ込めておくことが多いが、このときに閉じ込められた気体（分子の集まり）は一種の集団を形成している。この集団を**統計集団**と呼ぶ。例えば、この気体が外部との熱のやり取り、漏れ出しなどもなく完全に孤立している集団（**小正準集団**）だとすると、熱力学第1法則において外部との熱 $\mathrm{d}'Q$ と気体が行う仕事 $\mathrm{d}W$ はそれぞれ0となる（容器の体積が変化しないことから仕事も0）。そのため、内部エネルギーの変化量 $\mathrm{d}U$ は熱力学第一法則により

$$
    \mathrm{d}U=
    \mathrm{d}'Q+\mathrm{d}W=0
$$

というように0となる（内部エネルギーが保存される）ことが分かる。次に熱力学第二法則により、エントロピーの微小変化は $\mathrm{d}'Q$ が0であることから

$$
    \mathrm{d}S\ge\frac{\mathrm{d}'Q}{T}=0
$$

というように増大する方向に変化する。そのため、内部で熱平衡になるとエントロピーが最大になることが予想される。ここで、エントロピーはBoltzmannによると気体のとりうる状態数 $\mathcal{W}$ により

$$
    S=k_{\mathrm{B}}\ln\mathcal{W}
$$

というような関係があるとされている（ボルツマンの原理）ことから、エントロピーとともに状態数 $\mathcal{W}$ も最大になるともいえる。また、状態数というのは全体の状態がエネルギーを用いて書けることから計算することができる。具体的には、解析力学においてエネルギー $U$ はハミルトニアン $\mathcal{H}$ で記述される。例として、 $N$ 個の粒子がある場合は $i$ 番目の粒子の一成分の位置と運動量を $q_i,p_i\ (i=1,\cdots,3N)$ とすると
