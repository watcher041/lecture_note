...
$$
というような関係がある（ボルツマンの原理）。この状態数というのは全体の状態がエネルギーを用いて書けることから計算することができる。具体的には、解析力学においてエネルギー $U$ はハミルトニアン $\mathcal{H}$ で記述される。例として、 $N$ 個の粒子がある場合は $i$ 番目の粒子の一成分の位置と運動量を $q_i,p_i\ (i=1,\cdots,3N)$ とすると
$$
    U=\mathcal{H}
    (q_1,\cdots,q_{3N},p_1,
    \cdots,p_{3N})
...
$$
    ...
    \frac{p_i^2}{2m}
$$
の場合は、$p_1,\cdots,p_{3N}$ のみ依存している。簡単にするため、一旦粒子が一つで1次元の場合を考えると $q_1,p_1$ しかないため
$$
    \mathcal{H}=\frac{p_1^2}{2m}
$$
となる。上記の議論からエネルギーが一定になるということだったので、
$$
    p_1^2=2m U
$$
...
$$
    \mathcal{W}=\frac{2L}{h}
$$
である。ここで、$h$ は正確な状態数になるように定めた定数（長さ×運動量の次元を持つ）であり、後に登場する量子力学で重要なものとなっている。同じように次元を増やしていくことによって、$3N$ 次元になった場合は半径 $r$ の $f$ 次元の球の表面積が
$$
    S_{f}(r)=
