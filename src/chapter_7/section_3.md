...
$$
    ...
    \right]
$$
これらの式に基づいて、実際に重力がかかる場合の運動を考えてみる。重力には質量が関わってくるが、この質量というのは**重力質量**と呼ばれ、実質的には違うものとなっている。先ほどの質量は慣性（運動の状態を維持しようとする性質）がどのくらいかを表す**慣性質量**（加速のしにくさ）に相当している詳細は、また一般相対性理論の項で述べる）。そのため、運動方程式は以下の通りになる（。
$$
    \frac{\mathrm{d}}{\mathrm{d}t}
    \left(
        \frac{m_0\bm{v}}
